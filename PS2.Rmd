...
benfords_stats <- function(x, d=TRUE, m=TRUE){
  ...
  first_sigfigs <- as.integer(substr(as.character(as.integer(signif(x,1))),1,1))
  
  x_i <- rep(NA, 9)
  for(i in 1:9){
    x_i[i] <- sum(first_sigfigs==i)/length(x)
  }
  
  stats <- append(stats,
                  list("digit_distribution"=data.frame("digit"=1:9,
                                                       "proportion"=x_i)))
  
  if(d){
    cho_gains <- rep(NA,9)
    for(i in 1:9){
    cho_gains[i] <- (x_i[i]-log10(1+(1/i)))^2
  }  
    
    cho_gains <- sqrt(sum(cho_gains))
    stats <- append(stats, list("m_statistic"=cho_gains))
    
   }
  if(m){
    leemis <- rep(NA,9)
    for(i in 1:9){
      leemis[i] <- x_i[i]-log10((1+(1/i)))
    }
    
    leemis <- max(leemis)
    stats <- append(stats, list("d_statistic"=leemis))
...
print.benfords <- function(x){
  ...
  names(legend) <- "Significance"
  
  # Return in a list
  return(list(noquote(output), noquote(legend)))
